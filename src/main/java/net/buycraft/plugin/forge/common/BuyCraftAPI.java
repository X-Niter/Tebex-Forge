package net.buycraft.plugin.forge.common;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import net.buycraft.plugin.forge.common.data.Coupon;
import net.buycraft.plugin.forge.common.data.GiftCard;
import net.buycraft.plugin.forge.common.data.RecentPayment;
import net.buycraft.plugin.forge.common.data.ServerEvent;
import net.buycraft.plugin.forge.common.data.responses.*;
import okhttp3.*;
...
public interface BuyCraftAPI {
    ...
    @GET("/information")
    public retrofit2.Call<ServerInformation> getServerInformation();

    @GET("/listing")
    public retrofit2.Call<Listing> retrieveListing();

    @GET("/queue")
    public retrofit2.Call<DueQueueInformation> retrieveDueQueue();

    @GET("/queue/offline-commands")
    public retrofit2.Call<QueueInformation> retrieveOfflineQueue();

    @GET("/queue/online-commands/{id}")
    public retrofit2.Call<QueueInformation> getPlayerQueue(@Path("id") int id);

    @FormUrlEncoded
    @HTTP(method = "DELETE", path = "/queue", hasBody = true)
    public retrofit2.Call<Void> deleteCommands(@Field("ids[]") List<Integer> ids);

    @FormUrlEncoded
    @POST("/checkout")
    public retrofit2.Call<CheckoutUrlResponse> getCheckoutUri(@Field("username") String username, @Field("package_id") int packageId);

    @FormUrlEncoded
    @POST("/checkout")
    public retrofit2.Call<CheckoutUrlResponse> getCategoryUri(@Field("username") String username, @Field("category_id") int categoryId); //TODO Figure out category=true

    @GET("/payments")
    public retrofit2.Call<List<RecentPayment>> getRecentPayments(@Query("limit") int limit);

    @GET("/coupons")
    public retrofit2.Call<CouponListing> getAllCoupons();

    @GET("/coupons/{id}")
    public retrofit2.Call<Coupon> getCoupon(@Path("id") int id);

    @DELETE("/coupons/{id}")
    public retrofit2.Call<Void> deleteCoupon(@Path("id") int id);

    @DELETE("/coupons/{id}/code")
    public retrofit2.Call<Void> deleteCoupon(@Path("id") String id);

    public default retrofit2.Call<CouponSingleListing> createCoupon(Coupon coupon) {
    ...
    @POST("/coupons")
    public retrofit2.Call<CouponSingleListing> createCoupon(@Body RequestBody body);

    @GET("/gift-cards")
    public retrofit2.Call<GiftCardListing> getAllGiftCards();

    @GET("/gift-cards/{id}")
    public retrofit2.Call<GiftCardSingleListing> getGiftCard(@Path("id") int id);

    @DELETE("/gift-cards/{id}")
    public retrofit2.Call<GiftCardSingleListing> voidGiftCard(@Path("id") int id);

    public default retrofit2.Call<GiftCardSingleListing> topUpGiftCard(int id, BigDecimal amount) {
        return topUpGiftCard(id, new FormBody.Builder().add("amount", amount.toPlainString()).build());
    }

    @PUT("/gift-cards/{id}")
    public retrofit2.Call<GiftCardSingleListing> topUpGiftCard(@Path("id") int id, @Body RequestBody body);

    public default retrofit2.Call<GiftCardSingleListing> createGiftCard(BigDecimal amount, String note) {
    ...
    public default retrofit2.Call<GiftCardSingleListing> createGiftCard(GiftCard giftCard) {
        return createGiftCard(new FormBody.Builder()
    ...
    @POST("/gift-cards")
    public retrofit2.Call<GiftCardSingleListing> createGiftCard(@Body RequestBody body);

    @POST("/events")
    public retrofit2.Call<Void> sendEvents(@Body List<ServerEvent> events);
