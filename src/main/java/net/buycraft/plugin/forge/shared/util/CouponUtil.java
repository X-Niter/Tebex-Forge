package net.buycraft.plugin.forge.shared.util;
    ...
    public static long parseDuration(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (at == 'd' || at == 'w' || at == 'm' || at == 'h') {
                ...
                switch (at) {
                    case 'd' -> result += TimeUnit.DAYS.toMillis(n);
                    case 'w' -> result += TimeUnit.DAYS.toMillis(n * 7L);
                    case 'h' -> result += TimeUnit.HOURS.toMillis(n);
                    case 'm' -> result += TimeUnit.MINUTES.toMillis(n);
                }
